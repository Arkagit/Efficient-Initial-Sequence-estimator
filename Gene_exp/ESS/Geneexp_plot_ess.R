...
for(i in 1:B){
	for(j in 1:nloops){
		ess_track_bm[i,j] = as.numeric(Table[[i]][[j]][[1]])
		ess_track_ise[i,j] = as.numeric(Table[[i]][[j]][[2]])
		ess_track_cc[i,j] = as.numeric(Table[[i]][[j]][[3]])
		ess_track_sve[i,j] = as.numeric(Table[[i]][[j]][[4]])
	}
}
