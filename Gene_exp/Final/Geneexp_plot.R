...
for(i in 1:B){
	for(j in 1:nloops){
		print(j)
		ess_track_bm[i,j] = as.numeric(Table[[i]][[2*(j-1) + 1]][[1]])
		ess_track_ise[i,j] = as.numeric(Table[[i]][[2*(j-1) + 1]][[2]])
		ess_track_cc[i,j] = as.numeric(Table[[i]][[2*(j-1) + 1]][[3]])
		ess_track_sve[i,j] = as.numeric(Table[[i]][[2*(j-1) + 1]][[4]])

		time_track_bm[i,j] = as.numeric(Table[[i]][[2*(j-1) + 2]][[1]][3])
		time_track_ise[i,j] = as.numeric(Table[[i]][[2*(j-1) + 2]][[2]][3])
		time_track_cc[i,j] = as.numeric(Table[[i]][[2*(j-1) + 2]][[3]][3])
		time_track_sve[i,j] = as.numeric(Table[[i]][[2*(j-1) + 2]][[4]][3])
	}
}
...
# ESS plot
pdf("plot_ess_gene.pdf", height = 6, width = 6)
par(mar = c(5.1, 4.8, 4.1, 2.1))
plot(subsize, colMeans(ess_track_bm), ylim = c(0.94, 1.08),
...
# Time plot
pdf("plot_time_gene.pdf", height = 6, width = 6)
par(mar = c(5.1, 4.8, 4.1, 2.1))
plot(subsize, colMeans(time_track_bm), ylim = c(0, 600),
	type = 'l', xlab = "Time", ylab = "Time")
...
segments(x0 = subsize, y0 = colMeans(time_track_ise) - 1.96*se_time_ise, 
	y1 = colMeans(time_track_ise) + 1.96*se_time_ise, col = "green")
