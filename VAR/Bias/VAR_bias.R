...
var_coverage <- function(N = 1e3, phi, omega, B = 1000, level = .90)
{
  ...
  large.coverage <- matrix(0, nrow = B, ncol = 7)
  volume <- matrix(0, nrow = B, ncol = 7)
  ...
  for(b in 1:B){
    ...
    volume[b,6] <- det(bm)^(1/p)
    volume[b,7] <- det(mls)^(1/p)
    ...
    if(tester < large.crit) large.coverage[b,7] <- 1

  }
  colnames(large.coverage) <- c("Truth", "SVE", "LUG3", "New ISE (Geyer)", "ISE","BM", "New ISE (MLS)")
  colnames(volume) <- c("Truth", "SVE", "LUG3", "New ISE (Geyer)","ISE", "BM", "New ISE (MLS)")
  return(list("large" = large.coverage, "determinant" = volume, 
              "bias_bm" = bias_bm, "bias_sve" = bias_sve, "bias_lug" = bias_lug,
